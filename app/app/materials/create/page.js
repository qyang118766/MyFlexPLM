...
export default async function CreateMaterialPage({ searchParams }) {
  ...
  if (!userPermissions?.isAdmin && !['create', 'delete', 'full'].includes(materialPermission)) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Access Denied</h1>
          <p className="text-gray-600 mb-4">You do not have permission to create materials.</p>
          <Link
            href="/"
    ...
    if (!hasPermission) {
      return (
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center">
            <h1 className="text-2xl font-bold text-gray-900 mb-2">Access Denied</h1>
            <p className="text-gray-600 mb-4">You do not have permission to create materials of this type.</p>
            <Link
              href="/"
              className="inline-flex items-center justify-center rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700"
            >
  ...
  const attributeDefs = await getAttributesWithPermissions(supabase, 'material', typeId);
